...
It is highly recommended to read chapters 1, 2, 3, 4, 8, and 10 of Cracking The Coding Interview to familiarize yourself with the following data structures and their operations:  <br>
    Arrays: 
        1.翻转： lst.reverse() directly modifies the original list // lst[::-1]: a new copy list. 
        2.list.sort(reverse=True|False, key=myFunc) ascending 不返回任何东西, modifies the order of elements in the list.
          self.position.sort(key = lambda x:(x[1], x[0], x[2]))
        3.b = sorted(arr) 排序，但是不改变原有数组
        
    Maps  <br>
    Linked Lists  <br>
    Queues:  1. collections.deque()  2. q.append() 3. q.popleft() pop first element 4. q.pop() pop last element 5. q[0] get first element  <br>
