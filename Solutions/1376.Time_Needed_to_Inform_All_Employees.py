...
class Solution:
    def numOfMinutes(self, n: int, headID: int, manager: List[int], informTime: List[int]) -> int:
        def dfs(curr_node, curr_time):    #curr_time record the current time
            if informTime[curr_node] == 0:
                self.max_informtime = max(self.max_informtime, curr_time)
                
            for next_node in graph[curr_node]:
                dfs(next_node, curr_time + informTime[curr_node])
        
        #first we need build a graph for manager-sub constructor
        graph = defaultdict(list)
        for sub, mang in enumerate(manager):
            graph[mang].append(sub)
            
        self.max_informtime = 0   #recode global max informt
        dfs(headID, 0)
