...
class Solution:
    def delNodes(self, root: Optional[TreeNode], to_delete: List[int]) -> List[TreeNode]:
        ...
        ans = self.process(root, set_delete)
        if ans:
            self.res.append(ans)
        return self.res
    
    def process(self, root, set_delete):
        if not root:
            return None
        
        root.left = self.process(root.left, set_delete)
        root.right = self.process(root.right, set_delete)
        
        if root.val not in set_delete:
            return root
